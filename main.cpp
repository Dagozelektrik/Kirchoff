...
Mutex mutex_acc;
Mutex mutex_wait;
ConditionVariable cond(mutex_wait)
...
void controlCalculation()
{
    char pass_param_1[50];
    while(WHEEL_PPR_1 == 0.0){}
    int counter = 0;
    Timer t_control;
    while (1)
    {
        // t_control.start();
        mutex_wait.lock();
        Kernel::Clock::time_point end_time = Kernel::Clock::now() + 5ms;
...
//        xSemaphorePulseData.release();
        ...
        // Testing
        // nh.loginfo("---Logging---");
        // snprintf(pass_param_1, 20, "Current FL: %f", locomotion_FL_vel);
        // nh.loginfo(pass_param_1);
        // snprintf(pass_param_1, 20, "Target FL: %f", locomotion_FL_target_vel);
        // nh.loginfo(pass_param_1);
        ...
        counter++;
        // snprintf(pass_param_1, 50, "The time taken was %d (control)", t_control.read_ms());
        // nh.loginfo(pass_param_1);
        // int count_loop = 0;
        while (1){//t_control.read_ms() < 5) {
            // do nothing, wait until 5 ms
            // count_loop++;
        ...
        // snprintf(pass_param_1, 50, "The time taken was %d (control-after)", t_control.read_ms());
        // nh.loginfo(pass_param_1);
        // snprintf(pass_param_1, 50, "Loop count = %d", count_loop);
        // nh.loginfo(pass_param_1);
        t_control.reset();
        // count_loop = 0;
        // Thread::wait(5);
